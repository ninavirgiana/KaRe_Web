    ...
    public function getTabunganByDate(Request $request)
{
    ...
    // If input is not valid, return a response with status 400
    if ($validator->fails()) {
        return response()->json(['error' => $validator->errors()], 400);
    }
    ...
    // Query to get tabungan data by date
    $tabungan = Tabungan::whereDate('tgl_tabungan', $tanggal)->get();

    // If data is found, add user name to the response
    if ($tabungan->isNotEmpty()) {
        ...
        // If no data, return response with status 404 and a message

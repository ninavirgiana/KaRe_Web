    ...
    public function addAdmin(Request $request)
{
    ...
    $validator = Validator::make($request->all(), [
        ...
        'alamat_user' => 'required',
        'notelp_user' => 'required',
        'foto_user' => 'required|image|mimes:jpeg,png,jpg,gif', // Ukuran maksimum 2MB
    ]);
    ...
    if (User::where('email_user', $request->input('email_user'))->exists()) {
        return response()->json(['status' => 'error', 'message' => 'Email sudah terdaftar'], 400);
    }

    // Cek apakah nama sudah ada
    if (User::where('nama_user', $request->input('nama_user'))->exists()) {
        return response()->json(['status' => 'error', 'message' => 'Nama sudah ada'], 400);
    }
...
//MENAMBAHKAN ADMIN
    ...
    //         'alamat_user' => 'required',
    //         'notelp_user' => 'required',
    //         'foto_user' => 'required|image|mimes:jpeg,png,jpg,gif', // Pastikan foto adalah file gambar dengan format yang valid dan ukuran maksimum 2MB
    //     ]);
